...
const HeroSection = () => {
  ...
  const handleImageError = () => {
    setImageError(true);
    toast.error('Error cargando la imagen, usando valor por defecto');
  };

  // Determinar URL a mostrar
  const heroImageUrl = imageError
    ? PROFILE_IMAGE_URL
    : siteInfo?.heroImageUrl || PROFILE_IMAGE_URL;

  return (
    <section
      id="home"
      className="relative py-6 md:py-10 mt-6 md:mt-8 overflow-hidden"
    >
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          ...
          >
            ...
            <h1 className="font-clash font-bold text-4xl md:text-5xl lg:text-6xl text-primary mb-4 leading-tight">
              Transformando <span className="text-secondary">Datos</span> en{' '}
              <span className="text-secondary">Soluciones</span> Inteligentes
            </h1>
            <p className="text-base md:text-lg text-text/80 mb-6 max-w-lg">
              Transformo datos en decisiones estratégicas a través del análisis,
              la visualización y la mejora de procesos operativos. Con
              experiencia en la creación de dashboards en Power BI,
              automatización de reportes y soporte a la operatividad, hoy
              combino mi trayectoria analítica con formación en desarrollo full
              stack para construir soluciones tecnológicas que generan impacto
              real.
            </p>

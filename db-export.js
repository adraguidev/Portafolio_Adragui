...
async function exportDatabase() {
  ...
  try {
    ...
    for (const table of tables) {
      ...
      console.log(`✅ Exportados ${dataResult.rows.length} registros de ${table}`);
    }
    ...
    const lightExportFilename = path.join(backupsDir, `db-backup-light-${timestamp}.json`);
    ...
    if (lightData.articles) {
      lightData.articles.forEach(article => {
        if (article.content && article.content.length > 200) {
          article.content = article.content.substring(0, 200) + '... [contenido truncado]';
        }
      });
    }
